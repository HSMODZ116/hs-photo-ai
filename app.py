...
<body class="p-4 sm:p-8 min-h-screen flex items-center justify-center">
    ...
    <div id="appContainer" class="bg-white p-6 sm:p-10 rounded-xl shadow-2xl w-full max-w-4xl">
        ...
        <p class="text-center text-gray-500 mb-8">Upload your image, and transform it using an AI prompt. (Uses ImgBB for hosting)</p>
        ...
        <div class="space-y-6 mb-8">
            ...
            <div class="flex flex-col">
                ...
                <input type="text" id="promptInput" placeholder="Example: make this in cyberpunk style, high detail" value="" class="p-3 border border-gray-300 rounded-lg focus:ring-indigo-500 focus:border-indigo-500 transition duration-150">
            </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 border-t pt-8 border-gray-200">
            ...
            <div class="text-center">
                ...
                <div id="outputImageContainer" class="image-container mx-auto">
                    <img id="outputImage" src="https://placehold.co/400x400/9CA3AF/FFFFFF?text=Result+Will+Appear+Here" alt="Generated Image" class="w-full h-auto object-contain rounded-lg shadow-md border-2 border-green-500 aspect-square"
                         onerror="this.src='https://placehold.co/400x400/DC2626/FFFFFF?text=Image+Load+Failed'; document.getElementById('outputImage').alt='Image load failed due to invalid data or network issue'; showMessage('Error: Generated data was not a valid image. Check console for details.', 'bg-red-100 text-red-800'); this.parentElement.classList.remove('loaded');">
    ...
    <script>
        ...
        async function uploadImageToImgBB(file) {
            ...
            const resp = await fetch(uploadUrlWithKey, { method: 'POST', body: formData });
        ...
        function downloadTransformedImage() {
            if (transformedImageBlob) {
                ...
                a.href = url;
                a.download = 'ai_transformed_image_HS_Modz.png';
        ...
        async function processImage() {
            ...
            outputImage.onerror = function() { outputImage.src='https://placehold.co/400x400/DC2626/FFFFFF?text=Image+Load+Failed'; outputImage.alt='Image load failed due to invalid data or network issue'; showMessage('Error: Generated data was not a valid image.', 'bg-red-100 text-red-800'); outputImageContainer.classList.remove('loaded'); };
            ...
            processBtn.disabled=true; buttonText.textContent="Processing..."; buttonIcon.classList.add('animate-spin'); outputImage.src="https://placehold.co/400x400/E5E7EB/6B7280?text=Processing...";
            try {
                const publicImageUrl = await uploadImageToImgBB(file);
                ...
                if(imageBlob.size===0) throw new Error("Received empty response (0 bytes).");
                transformedImageBlob=imageBlob;
            ...
            } catch(error){ console.error(error); showMessage(`Transformation failed: ${error.message}`, 'bg-red-100 text-red-800'); }
